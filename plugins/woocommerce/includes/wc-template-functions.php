...
if ( ! function_exists( 'wc_get_variable_product_lazy_variation_data' ) ) {

	/**
	 * Get compact variation data for matching in the classic add-to-cart form.
	 *
	 * This intentionally avoids the rendered fields generated by
	 * WC_Product_Variable::get_available_variation(). The full payload is fetched
	 * from the existing get_variation AJAX endpoint after a concrete selection is
	 * made.
	 *
	 * @since 10.10.0
	 ...
	 * @return array[]
	 */
	function wc_get_variable_product_lazy_variation_data( $product ) {
		...
		foreach ( $product->get_available_variations( 'objects' ) as $variation ) {
			...
			/**
			 * Filters compact variation data used by the classic add-to-cart form.
			 *
			 * @since 10.10.0
			 *
			 * @param array                $variation_data Compact variation data.
			 * @param WC_Product_Variable  $product        Parent variable product.
			 * @param WC_Product_Variation $variation      Variation product.
			 */
			$variation_data = apply_filters(
				'woocommerce_lazy_available_variation',
				array(
...
if ( ! function_exists( 'woocommerce_variable_add_to_cart' ) ) {
	...
	function woocommerce_variable_add_to_cart() {
		...
		$ajax_variation_threshold = apply_filters( 'woocommerce_ajax_variation_threshold', 30, $product );

		// Get Available variations?
		$get_variations = $variation_count <= $ajax_variation_threshold;

		/**
		 * Filters the variation count above which classic variable forms inline
		 * compact variation maps and fetch the full variation payload lazily.
		 *
		 * This threshold only applies while the product is still below the
		 * `woocommerce_ajax_variation_threshold`. The default matches the built-in
		 * AJAX threshold, preserving current behavior unless a site raises the AJAX
		 * threshold for larger variable products.
		 *
		 * @since 10.10.0
		 *
		 * @param int        $threshold                Variation count threshold.
		 * @param WC_Product $product                  Product object.
		 * @param int        $ajax_variation_threshold AJAX variation threshold.
		 */
		$lazy_variation_threshold = apply_filters( 'woocommerce_lazy_variation_data_threshold', 30, $product, $ajax_variation_threshold );
		$get_lazy_variations      = $get_variations && $variation_count > $lazy_variation_threshold;

		// Load the template.
		wc_get_template(
			'single-product/add-to-cart/variable.php',
			array(
				'available_variations'   => $get_variations ? ( $get_lazy_variations ? wc_get_variable_product_lazy_variation_data( $product ) : $product->get_available_variations() ) : false,
				'attributes'             => $product->get_variation_attributes(),
				'is_lazy_variation_data' => $get_lazy_variations,
